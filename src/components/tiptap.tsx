...
import { Bold, Italic, Strikethrough, Underline as UnderlineIcon, Link2, Quote, Code, Plus, Image as ImageIcon, ImagePlus } from 'lucide-react'
...
const CustomBubbleMenu = ({editor}: {editor: Editor}) => {
  return (
    <BubbleMenu tippyOptions={{ duration: 200, animation: "shift-away-subtle" }} editor={editor}>
      <Menubar>
...
const CustomFloatingMenu = ({editor}: {editor: Editor}) => {
  return (
    <>
      {editor && <FloatingMenu className='relative -left-20' tippyOptions={{ duration: 200, animation: "shift-away-subtle" }} editor={editor}>
        <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="icon" className='rounded-full p-2' onMouseEnter={(e) => {
          ...
          <DropdownMenuContent side='right' align='start' sideOffset={12}>
            ...
            <DropdownMenuItem className='gap-x-2' onClick={() => {
              editor.commands.insertContent("<blockquote><p></p></blockquote>")
            }}>
              <Quote className='w-4 h-4' />
              引用
            </DropdownMenuItem>
            <DropdownMenuItem className='gap-x-2' onClick={() => {
              editor.commands.insertContent('<pre><code class="language-prompt"></code></pre>')
            }}>
              <Code className='w-4 h-4' />
              コードブロック
            </DropdownMenuItem>
